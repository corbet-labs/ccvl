...
The command validates both keys, copies `templates/application.json`, assigns a
stable ID, and refuses to overwrite an existing record. Schema version 4
contains:
...
- when enabled, exactly six measured paragraphs following
  `docs/cover-letter.md` and exactly five one-line highlights.
