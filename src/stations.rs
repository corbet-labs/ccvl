...
pub fn load_plan(workspace: &Workspace, path: &Path) -> Result<Value> {
    validate_json_file(path, &workspace.path("schemas/stations.schema.json"))
}
...
pub fn validate_general(workspace: &Workspace, require_ready: bool) -> Result<Assessment> {
    let plan_path = workspace.path("cvl/general/stations.json");
    let document = load_plan(workspace, &plan_path)?;
    let assessment = assess(workspace, &document, "cvl/general/stations.json")?;
    ...
    let de = verify_source_counts(
        workspace,
        &document,
        &workspace.path("cvl/general/de-ch/cv.typ"),
    )?;
    let en = verify_source_counts(
        workspace,
        &document,
        &workspace.path("cvl/general/en-ch/cv.typ"),
    )?;
    ensure!(
