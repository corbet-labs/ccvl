...
`.agent/docs/public-identifiers.md`. Everything else is private by default.
...
- real handwritten signatures, street addresses, dates of birth, or identity
  files;
- private company, role, and fit research;
...
the platform `public-check` command. A deny-list cannot understand context; a
human or agent must still inspect the complete staged tree and diff before
publication.
